...
def main(pow_val: int):
    try:
        spark = (
            SparkSession.builder.appName(
                f"PySpark Game of Life (pow={pow_val})"
            )
            .master("local[*]")
            .getOrCreate()
        )
        ...
        logging.error(
            "Ocorreu um erro durante a execução do Spark.", exc_info=True
        )
    finally:
        if "spark" in locals():
            spark.stop()
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Uso: python jogo_da_vida.py <POW>")
        sys.exit(1)
    try:
        pow_val = int(sys.argv[1])
    except ValueError:
        print("Erro: POW deve ser um inteiro")
        sys.exit(1)
    main(pow_val)
